		...
		load_markers = function (map, map_use_icon, map_use_url_icon) {
			...
			getHtmlInfoWindow = function (untappd_checkin) {
				...
				if (untappd_checkin.rating_score) {
					rating_score = $("<div>").attr({ class: 'checkin_rating' }).append(
						$("<h5>").text('Rating:')
					).append(
				...
				if (untappd_checkin.comment) {
					checkin_comment = $("<div>").attr({ class: 'checkin_comment' }).append(
						$("<h5>").text(ajax_untappd_config.languages[1] + ':'))
						.append(
							$("<p>").html($("<b>").text(untappd_checkin.comment))
						);
				...
				var html_data = $("<div>").attr({ class: 'infowindow-content' }).append(
					html_title
				).append(
					html_image
				).append(
